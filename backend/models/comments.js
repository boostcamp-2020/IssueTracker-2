...
exports.create = async ({ issueId, writerId, description }) => {
  try {
    ...
    const [{ generatedCommentId }] = await connection.query(sql1, [writerId, description]);

    let sql2 = 'INSERT INTO issue_comments (issue_id, comment_id) VALUES (?, ?)';
    const [{ generatedIssueCommentId  }] = await connection.query(sql2, [issueId, commentId]);
    
    connection.release();
    
    return { generatedCommentId, generatedIssueCommentId };
